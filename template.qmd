...
```{r}
# making a vector of what we want the column names. 
var_names<-c('a', 'p', 'c', 'l_k', 'w_k', 'asym', 'l_g', 'variety')
temp<-read_tsv(here::here('data','seeds_dataset.txt'),
               #col_names = FALSE means there are no column names currently in the data frame
                     col_names = FALSE) |> 
  #just the names to the variable names in the created vector, has to match in number
  setNames(var_names)
```


In your console use `summary(temp)` to examine the structure of the data. Does anything look strange?
-999.00 values. variety is classed as a number in the data frame
...
seeds_df <- read_tsv(here::here('data','seeds_dataset.txt'),
                     col_names = FALSE,
                     # na = '-999 we're telling r that this specific value (-999.00) is representing NA
                     na = '-999') %>%
  setNames(var_names) %>%
  #in the column variety, if something is exactly equal to 1, it should be Kama, ect, if there's not a value outside of 1, 2, 3, make variety oops
...
```{r}
#histogram
a_histogram <- ggplot(seeds_df, aes(x = a)) + 
  geom_histogram()  # This will create a histogram of the 'a' variable
a_histogram

p_histogram <- ggplot(seeds_df, aes(x = p)) + 
  geom_histogram()  # This will create a histogram of the 'a' variable
p_histogram

lk_histogram <- ggplot(seeds_df, aes(x = l_k)) + 
  geom_histogram()  # This will create a histogram of the 'a' variable
lk_histogram

kernal_a <- ggplot(seeds_df, aes(x = a, y = asym))+
  geom_point()
kernal_a

kernal_lk <- ggplot(seeds_df, aes(x = l_k, y = asym))+
  geom_point()
kernal_lk
...
seeds_complete <- seeds_df |>
  #drop na drops any rows with na values from the data frame
  drop_na()

seeds_scale <- seeds_complete |>
  #select and then remove (-) the variety column from the data frame
  select(-variety) |> 
  # For each numeric column, the mean of the column is subtracted from every value in that column
  scale() 
```
...
#take the scaled data, and then what's the range of clusters we think exists in the data. minimum of 2, maxiumum of 10. 
...
set.seed(10101)
seeds_km <- kmeans(seeds_scale, 3, nstart = 25) 
# 
# kmeans specifying 3 groups to start
```
...
```{r}
# mmake a new data frame called seeds_cl using data from the seeds_complete dataframe
seeds_cl <- data.frame(seeds_complete,
                       cluster_num = factor(seeds_km$cluster))
```
...
a_histogram <- ggplot(seeds_cl, aes(x = a, color = cluster_num)) + 
  geom_histogram()  # This will create a histogram of the 'a' variable
a_histogram


kernal_lk <- ggplot(seeds_cl, aes(x = l_k, y = asym, color = cluster_num))+
  geom_point()
kernal_lk
```
...
```{r}
#| tlb-cap: contiency table for the groups of kmeans and the varietals of kernels
seeds_cl |>
  select(variety, cluster_num) |>
  table()|>
  knitr::kable()
```
...
1. get the top 20 emmitters
2. drop na values if any
3. select only numeric
4. scale data
...
wb_ghg_20 <- wb_env |>
  #from the ghg column, take the top 20 values
  slice_max(ghg, n=20)
...
wb_scaled <- wb_ghg_20 |>
  #remove the nonnumeric columns
  select(-name, -region )|>
  scale()

#name the rows based on the name from the name column
